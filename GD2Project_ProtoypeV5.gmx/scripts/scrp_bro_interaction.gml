//interact with bros
//when you approach a bro, check who that bro is
if (distance_to_object(obj_bro_dancer) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_dancing_bro_chat();
    global.portrait = obj_bro_dancer;
}

else if (distance_to_object(obj_tweak) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_tweak_chat();
    global.portrait = obj_tweak;
}

else if (distance_to_object(obj_painter) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_painter_chat();
    global.portrait = obj_painter;
}

else if (distance_to_object(obj_tanning_bro) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_tanning_bro_chat();
    global.portrait = obj_tanning_bro;
}

else if (distance_to_object(obj_dad_bro) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_gym_father_chat();
    global.portrait = obj_dad_bro;
}
else if (distance_to_object(obj_mom_bro) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_gym_mother_chat();
    global.portrait = obj_mom_bro;
}
else if (distance_to_object(obj_quote_bro_1) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_quote_bro_1_chat();
    global.portrait = obj_quote_bro_1;
}
else if (distance_to_object(obj_quote_bro_2) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_quote_bro_2_chat();
    global.portrait = obj_quote_bro_2;
}
else if (distance_to_object(obj_quote_bro_3) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_quote_bro_3_chat();
    global.portrait = obj_quote_bro_3;
}
else if (distance_to_object(obj_quote_bro_4) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_quote_bro_4_chat();
    global.portrait = obj_quote_bro_4;
}
else if (distance_to_object(obj_quote_bro_5) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_quote_bro_5_chat();
    global.portrait = obj_quote_bro_5;
}
else if (distance_to_object(obj_quote_bro_6) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_quote_bro_6_chat();
    global.portrait = obj_quote_bro_6;
}
else if (distance_to_object(obj_L3_word_puzzle_bro) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_L3_word_puzzle_bro_chat();
    global.portrait = obj_L3_word_puzzle_bro;
}
else if (distance_to_object(obj_broflex_bro) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_broflex_bro_chat();
    global.portrait = obj_broflex_bro;
}
else if (distance_to_object(bro_service_desk) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_desk_bro_chat();
    global.portrait = bro_service_desk;
}
else if (distance_to_object(obj_brotien_salesman) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_brotien_salesman();
    global.portrait = obj_brotien_salesman;
}
else if (distance_to_object(obj_brobi) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_tmp_brobi1();
    global.portrait = obj_brobi;
}
else if (distance_to_object(obj_bro_1) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_locker_bro_chat();
    global.portrait = obj_bro_1;
}
else if (distance_to_object(obj_bro_4) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_steroids_bro_chat();
    global.portrait = obj_bro_4;
}
else if (distance_to_object(obj_bro_baby) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_baby_bro_chat();
    global.portrait = obj_bro_baby;
}
else if (distance_to_object(obj_bro_life) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_lifeguard_bro_chat();
    global.portrait = obj_bro_life;
}
else if (distance_to_object(obj_plantyMcplantface) < 15) {
    audio_play_sound(snd_interaction, 15, false);
    scrp_plantymcplantface_chat();
    global.portrait = obj_plantyMcplantface;
}
